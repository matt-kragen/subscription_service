  ...
  describe 'happy path' do
    describe 'POST /subscriptions' do
      it 'allows a customer to create a tea subscription' do
        ...
        expect(subscription[:data][:attributes].keys).to eq(%i[title price status frequency customer_id
                                                               tea_id])
      end
    end
    ...
    describe 'PATCH /subscriptions' do
      it 'can cancel an existing subscription' do
        ...
        expect(updated_subscription[:data][:attributes].keys).to eq(%i[title price status frequency customer_id
                                                                       tea_id])
    ...
    describe 'GET /subscriptions' do
      it 'lists all subscriptions for a customer including cancelled' do
        sub1 = Subscription.create(customer_id: @customer1.id, tea_id: @tea1.id, title: 'First Subscription',
                                   price: 5.00, status: 'Active', frequency: 1)
        sub2 = Subscription.create(customer_id: @customer1.id, tea_id: @tea2.id, title: 'Second Subscription',
                                   price: 10.00, status: 'Cancelled', frequency: 2)
